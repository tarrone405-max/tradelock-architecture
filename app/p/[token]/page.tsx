...
import { signChangeOrder, declineChangeOrder, payChangeOrder } from "./actions";
...
export default async function ClientPortalPage({
  ...
  // No Supabase Auth session exists for homeowners, so this page (and every
  // action in ./actions.ts) validates the token itself against the database
  // via the service-role client, rather than relying on the client-token
  // RLS policies from Phase 2 — see the comment atop actions.ts.
  ...
  const companyName = project.users?.company_name;
  const providerAcceptsPayments = !!project.users?.stripe_connect_charges_enabled;

  return (
    <div className="min-h-screen bg-gray-50 px-4 py-8 sm:py-12">
      <div className="mx-auto max-w-lg space-y-6">
        <div className="text-center">
          {companyName && <p className="text-sm font-medium text-gray-900">{companyName}</p>}
          <h1 className="text-xl font-semibold text-gray-900">{project.client_name}</h1>
        ...
        {activeTerms && (
          <div className="rounded-xl border border-gray-200 bg-white p-4">
            <h2 className="text-sm font-semibold text-gray-900">Terms of Service</h2>
            <p className="mt-0.5 text-xs text-gray-900">Version {activeTerms.version}</p>
        ...
        {orders.length === 0 ? (
          ...
          <ul className="space-y-4">
            {orders.map((co) => {
              const hasApprovalRecord = co.status !== "pending" && co.status !== "declined";
              return (
              <li key={co.id} className="rounded-xl border border-gray-200 bg-white p-5">
                ...
                <div className="mt-2 flex items-center justify-between gap-3">
                  <div>
                    <p className="text-lg font-semibold text-gray-900">
                      {currency.format(Number(co.cost))}
                    </p>
                    {co.due_date && (
                ...
                {co.status === "pending" && (
                  <div className="mt-4 space-y-3 border-t border-gray-100 pt-4">
                    ...
                    />
                    <form action={declineChangeOrder} className="text-center">
                  ...
                  <div className="mt-4 border-t border-gray-100 pt-4">
                    {!co.provider_signed_at ? (
                      ...
                      <form action={payChangeOrder}>
                        ...
                        >
                          Pay {currency.format(Number(co.cost))}
                        </button>
                      </form>
                    ) : (
                      <p className="text-center text-xs text-gray-900">
                        Online payment isn&rsquo;t set up yet — please contact{" "}
                        {companyName || "your contractor"} directly to pay this change order.
                      </p>
                    )}
                  </div>
                )}
        ...
        <p className="text-center text-xs text-gray-900">Secured by TradeLock</p>
      </div>
    </div>
  );
}
